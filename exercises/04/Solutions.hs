...
listFromRange n m
  | n > m = []
  | otherwise = n : listFromRange (succ n) m
...
divisors n = [x | x <- listFromRange 1 n, x `divides` n]
  where
    divides x m = m `rem` x == 0
...
-- alternatively
-- cartesian [] _ = []
-- cartesian (x:xs) ys = megaPair x ys `append` cartesian xs ys
...
digitsToNumber = go . reverse
  where
    ...
    go [] = 0
    go (n : ns) = traceShow n $ n + 10 * go ns
...
transpose (xs : xss) = zipWith (\x txss -> x : txss) xs $ transpose xss
...
reverseLinear = go []
  where
    -- we use the additional helper to introduce a "variable" in which we accumulate values
