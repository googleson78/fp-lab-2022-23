...
bracketState before after act = before *> act <* after
...
instance Functor (State s) where
  ...
  fmap f mx = MkState $ \st ->
    let (newState, x) = runState mx st
     in (newState, f x)
...
instance Applicative (State s) where
  ...
  liftA2 f mx my = MkState $ \initState ->
    let (mxState, x) = runState mx initState
        (myState, y) = runState my mxState
     in (myState, f x y)
...
instance Monad (State s) where
  ...
  -- HINT: use let bindings
  mx >>= f = MkState $ \initState ->
    let (mxState, x) = runState mx initState
     in runState (f x) mxState
...
runStatement (Print x) = do
  xExpr <- lookupEnv x
  case xExpr of
    Nothing -> pure $ Just $ x ++ " was not found!"
    Just expr -> do
      res <- eval expr
      pure $ Just $ show res
runStatement (Bind x expr) = do
  extendEnv x expr
  pure Nothing
...
runInState = usingState [] . fmap catMaybes . traverse runStatement
...
bindWithJoin mx f = join $ fmap f mx
