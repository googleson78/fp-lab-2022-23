...
Ще го използваме главно като индекс в "ред"/матрица.
...
Тип данни който представя "три `a`-та" използвайки функция.
Трите `a`-та можем да "получим" извиквайки съответно `f Zero`, `f One` и `f Two`.
...
thriceToTriple t = MkTriple (f Zero) (f One) (f Two)
...
Подобно, имаме и функция която го обръща във форма, която лесно се показва в `ghci`, тъй като
функциите по подразбиране не могат да бъдат принени:
```haskell
...
между `Thrice (Thrice a)` и `Matrix a` за да можем да представим абстрактен интърфейс за работа
с външния свят.
...
Матрица, представена чрез "ред от редове" - в този случай инцидентно това "ред" == `Thrice`.

Или ако разкрием типовия синоним, виждаме че това е `Three -> Three -> a` - по дадени два индекса,
получаваме елемент на матрицата.

Подобно на `Thrice`, елементите получаваме като извикаме функцията.

Например ако искаме елемента на 2 ред и 3 колонка от матрицата `m` можем да я извикаме по следния начин - `m One Two`.
...
т.е. нещо подобно на `std::unordered_map` или `std::map` от `C++`.

Тогава има доста директна аналогия между `Map k v` и `k -> v` - и двете са начини по `k`
...
* във всичките примери които давам по-надолу ще съм си избрал моята собствена имплементация
  на принтене за да демонстрирам резултата от функции, но не е нужно вашата да е същата.
...
Има *още един* по-императивен начин да си мислим за това, който е че за всеки индекс(`Zero`, `One`, `Two`) изпълняваме някаква операция (`f`), използвайки `op` за да комбинираме резултатите накрая.
...
### Конструиране
Измислете функция, подобна на `thrice` и `matrix`, която да позволява по-лесно построяване на този тип данни.
...
### map
Реализирайте `map` функция за `GenericMatrix`, т.е. нещо което взима поне `(a -> b)` както и `GenericMatrix` съдържащо `a`-та и връща
